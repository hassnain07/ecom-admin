                  ...
                  <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                      ...
                      <div class="col-sm-10">
                        ...
                        <small class="text-muted">Upload one primary image (leave empty to keep existing)</small>
                        <div id="primaryPreview" class="mt-2">
                          @if($product->image)
                            <img src="{{ asset('uploads/products/primary/'.$product->image) }}" class="img-thumbnail" style="max-width:150px;">
                          @endif
                        </div>
                      </div>
                    </div>
                   ...
                   <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input type="file" name="secondary_images[]" class="form-control" multiple>
                        <div class="mt-2 d-flex flex-wrap gap-2">
                        @foreach($product->images as $img)
                            <img src="{{ asset('uploads/products/secondary/'.$img->path) }}" 
                ...
                {{-- Description --}}
                    ...
                    <div class="col-sm-10">
                        ...
                        @if($product->variations->count())
                            ...
                            @php $values = json_decode($variation->values, true) ?? []; @endphp
                            <div class="variation-group mb-3 border p-3 rounded">
                                <div class="mb-2 d-flex justify-content-between align-items-center">
                                    <input type="text" name="variations[{{ $vIndex }}][name]" class="form-control me-2" value="{{ $variation->name }}">
                                ...
                                <div class="values-wrapper">
                                    ...
                                    <div class="value-row d-flex mb-2">
                                        <input type="text" name="variations[{{ $vIndex }}][values][{{ $valIndex }}][value]" class="form-control me-2" value="{{ $val['value'] ?? '' }}">
                                        <input type="number" step="0.01" name="variations[{{ $vIndex }}][values][{{ $valIndex }}][price]" class="form-control me-2" value="{{ $val['price'] ?? '' }}">
                        ...
                        <div class="variation-group mb-3 border p-3 rounded">
                            <div class="mb-2 d-flex justify-content-between align-items-center">
                                <input type="text" name="variations[0][name]" class="form-control me-2" placeholder="Variation Name (e.g., Size)">
                                <button type="button" class="btn btn-sm btn-danger remove-variation">Remove</button>
                            </div>
                            <div class="values-wrapper">
                                <div class="value-row d-flex mb-2">
...
<script>
 ...
 $(document).ready(function() {
    ...
    let variationIndex = document.querySelectorAll('.variation-group').length || 1;
    ...
    newGroup.innerHTML = `
        <div class="mb-2 d-flex justify-content-between align-items-center">
        <input type="text" name="variations[${variationIndex}][name]" class="form-control me-2" placeholder="Variation Name (e.g., Color)">
        <button type="button" class="btn btn-sm btn-danger remove-variation">Remove</button>
        </div>
        <div class="values-wrapper">
        <div class="value-row d-flex mb-2">
...
// Event delegation for Add/Remove Value & Remove Variation
document.getElementById('variation-wrapper').addEventListener('click', function (e) {
  if (e.target.classList.contains('add-value')) {
    ...
    let groupIndex = varNameInput.match(/variations\[(\d+)\]/)[1]; 
