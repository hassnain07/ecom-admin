...
$(document).on('click', '.delete-button', function() {
    ...
    var formAction = '{{ route("products.destroy", ":id") }}'; // Placeholder route
...
$(document).ready(function () {
    ...
    ajax: "{{ url('products-data') }}", // Fetching products data via AJAX
    columns: [
        { 
            data: 'id', 
            name: 'checkbox', 
        ...
        { data: "store_name", name: "store_name" },    // Store Name
        { data: "price", name: "price" },              // Price
        { 
            data: "status", 
            name: "status",
    ...
    $('#bulk-delete').on('click', function () {
        ...
        if (selectedIds.length > 0) {
            ...
            $('#confirmDelete').off('click').on('click', function () {
                $.ajax({
                    url: '{{ route("products.bulkDelete") }}',  // Route for bulk delete
                    type: 'POST',
                    data: {
                        ids: selectedIds,
                        _token: '{{ csrf_token() }}'
