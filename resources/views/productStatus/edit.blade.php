...
@section('title', 'Product Status | Edit')
@section('content')
                            ...
                            <select 
                              ...
                              @foreach($products as $product)
                                <option value="{{ $product->id }}" 
                                  {{ (old('product_id', $productStatus->product_id) == $product->id) ? 'selected' : '' }}>
                                  {{ $product->name }}
                                </option>
                              @endforeach
                            </select>
                            ...
                            <select 
                              ...
                              @foreach($statuses as $status)
                                <option value="{{ $status->id }}" 
                                  {{ (old('status_id', $productStatus->status_id) == $status->id) ? 'selected' : '' }}>
                                  {{ $status->name }}
                                </option>
                              @endforeach
                            </select>
                        ...
                        <div class="row mb-6">
                          ...
                          <div class="col-sm-10">
                            <input 
                              ...
                              value="{{ old('sale_price', $productStatus->sale_price) }}"
                              step="0.01"
                              min="0"
