                    ...
                    <table class="table table-hover datatable" id="datatable">
                        <thead>
                            <tr>
                                ...
                                <th class="col-md-1 py-4">Name :</th>
                                <th class="col-md-2 py-4">Service Img</th>
                                <th class="col-md-2 py-4">Service Description</th>
...
$(document).on('click', '.delete-button', function() {
    ...
    var formAction = '{{ route("services.destroy", ":id") }}'; // Placeholder route
...
$(document).ready(function () {
    let table = $("#datatable").DataTable({
        ...
        ajax: "{{ url('services-data') }}", // Fetching users data via AJAX
        columns: [
            { 
                data: 'id', 
                name: 'checkbox', 
            ...
            { data: "name", name: "name" },  // Sr No
            { data: "image", name: "image" },  // User
            { data: "short_desc", name: "short_desc" },  // Email
            { 
                data: "action", 
                name: "action", 
                orderable: false, 
    ...
    $('#bulk-delete').on('click', function () {
        ...
        if (selectedIds.length > 0) {
            ...
            $('#confirmDelete').off('click').on('click', function () {
                $.ajax({
                    url: '{{ route("services.bulkDelete") }}',  // Route for bulk delete
                    type: 'POST',
                    data: {
                        ids: selectedIds,
                        _token: '{{ csrf_token() }}'
