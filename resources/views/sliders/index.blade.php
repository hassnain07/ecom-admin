                    ...
                    <table class="table table-hover datatable" id="datatable">
                        <thead>
                            <tr>
                                ...
                                <th class="col-md-2 py-4">Slider Img</th>
                                <th class="col-md-2 py-4">Slider Text</th>
...
$(document).on('click', '.delete-button', function() {
    ...
    var formAction = '{{ route("sliders.destroy", ":id") }}'; // Placeholder route
...
$(document).ready(function () {
    let table = $("#datatable").DataTable({
        ...
        ajax: "{{ url('sliders-data') }}", // Fetching users data via AJAX
        columns: [
            { 
                data: 'id', 
                name: 'checkbox', 
            ...
            { data: "sliderImg", name: "sliderImg" },  // User
            { data: "sliderTxt", name: "sliderTxt" },  // Email
            { 
                data: "action", 
                name: "action", 
                orderable: false, 
    ...
    $('#bulk-delete').on('click', function () {
        ...
        if (selectedIds.length > 0) {
            ...
            $('#confirmDelete').off('click').on('click', function () {
                $.ajax({
                    url: '{{ route("sliders.bulkDelete") }}',  // Route for bulk delete
                    type: 'POST',
                    data: {
                        ids: selectedIds,
                        _token: '{{ csrf_token() }}'
