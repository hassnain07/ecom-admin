                    ...
                    <table class="table table-hover datatable" id="datatable">
                        <thead>
                            <tr>
                                ...
                                <th class="col-md-1 py-4">Status Name :</th>
                                <th class="col-md-1 py-4">Status Label :</th>
...
$(document).on('click', '.delete-button', function() {
    ...
    var formAction = '{{ route("status.destroy", ":id") }}'; // Placeholder route
...
$(document).ready(function () {
    let table = $("#datatable").DataTable({
        ...
        ajax: "{{ url('status-data') }}", // Fetching users data via AJAX
        columns: [
            { 
                data: 'id', 
                name: 'checkbox', 
    ...
    $('#bulk-delete').on('click', function () {
        ...
        if (selectedIds.length > 0) {
            ...
            $('#confirmDelete').off('click').on('click', function () {
                $.ajax({
                    url: '{{ route("status.bulkDelete") }}',  // Route for bulk delete
                    type: 'POST',
                    data: {
                        ids: selectedIds,
                        _token: '{{ csrf_token() }}'
